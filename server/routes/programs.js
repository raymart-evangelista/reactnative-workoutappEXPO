...
programsRouter.delete("/:id", async (req, res) => {
  try {
    ...
    console.log(
      `Program with name: ${program.name} and id: ${program._id} has been deleted.`,
    );
    res.send(
      `Program with name: ${program.name} and id: ${program._id} has been deleted.`,
    );
...
programsRouter.patch("/:id", async (req, res) => {
  try {
    ...
    const warmupSetsNeedCompute = warmupSetsAreDifferent(
      currentProgram,
      req.body,
    );
    const workingSetsNeedCompute = workingSetsAreDifferent(
      currentProgram,
      req.body,
    );
        ...
        week.dayDetails.forEach((day, dayIndex) => {
          day.exercises.forEach((exercise, exerciseIndex) => {
            exercise.warmupSetsCompletion = {
              individual: new Array(Number(exercise.warmupSets.max)).fill(
                false,
              ),
              overall: false,
            };
        ...
        week.dayDetails.forEach((day, dayIndex) => {
          day.exercises.forEach((exercise, exerciseIndex) => {
            exercise.workingSetsCompletion = {
              individual: new Array(Number(exercise.workingSets.max)).fill(
                false,
              ),
              overall: false,
            };
