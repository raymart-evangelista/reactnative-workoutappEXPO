...
const CollapsibleCard = ({
  ...
  const animatedGestureTapAreaStyle = useAnimatedStyle(() => {
    return {
      borderBottomLeftRadius: withTiming(pressed.value ? 0 : theme.roundness, {
        duration: 200,
      }),
      borderBottomRightRadius: withTiming(pressed.value ? 0 : theme.roundness, {
        duration: 200,
      }),
    };
  });
    ...
    <View style={cardContainerStyle}>
      <PaperCard style={cardStyle} mode="elevated">
        <GestureDetector gesture={tap}>
          <TouchableOpacity onLongPress={onLongPress} disabled={disabled}>
            <Animated.View
